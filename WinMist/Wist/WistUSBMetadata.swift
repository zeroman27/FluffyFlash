...
//  Wist
//
//  Sidecar JSON on the FAT32 volume (written by Wist).
//

import Foundation

/// Written to `Wist.meta.json` at the root of the `WINSETUP` volume before eject.
struct WistUSBMetadata: Codable, Equatable, Hashable, Sendable {
    static let fileName = "Wist.meta.json"
    ...
    /// Tries `Wist.meta.json`, then legacy `WinForge.meta.json` from older builds.
    static func read(from volumeRoot: URL) -> WistUSBMetadata? {
        let candidates = [fileName, "WinForge.meta.json"]
