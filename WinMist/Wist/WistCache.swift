...
struct UUPCacheFolder: Identifiable, Hashable {
    ...
    /// Written when a UUP download completes (`Wist.cache.json`), or inferred from sibling UUP folder for `*-iso-build` dirs.
...
enum WistCache: Sendable {
    ...
    static var uupRootDirectory: URL {
        FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Wist/UUP", isDirectory: true)
    }

    /// Scans `~/Library/Caches/Wist/UUP/<uuid>/`.
    ...
    /// Reads `Wist.cache.json` in the folder, or from the sibling UUP folder when `name` ends with `-iso-build`.
