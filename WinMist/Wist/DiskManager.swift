...
struct RemovableDriveInfo: Identifiable, Hashable {
    ...
    /// Parsed from `Wist.meta.json` on a mounted volume belonging to this whole disk, if present.
...
final class DiskManager: ObservableObject {
    ...
    /// Scans `/Volumes` for `Wist.meta.json`, maps each file to a whole-disk id via `diskutil info`, merges into `drives`.
    nonisolated private static func attachWistSidecarMeta(to drives: [RemovableDriveInfo]) -> [RemovableDriveInfo] {
        ...
        guard !metaByWholeDisk.isEmpty else { return drives }
