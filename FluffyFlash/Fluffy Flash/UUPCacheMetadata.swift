...
struct UUPCacheMetadata: Codable, Equatable, Hashable, Sendable {
    static let fileName = "FluffyFlash.cache.json"
    ...
    static func read(from uupDirectory: URL) -> UUPCacheMetadata? {
        let names = [fileName, "Wist.cache.json"]
        for name in names {
            let url = uupDirectory.appendingPathComponent(name)
            guard let data = try? Data(contentsOf: url) else { continue }
            if let meta = try? JSONDecoder().decode(UUPCacheMetadata.self, from: data) {
                return meta
            }
        }
        return nil
