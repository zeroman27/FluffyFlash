...
actor Downloader {
    private final class Delegate: NSObject, URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    private func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        guard let continuation = downloads.removeValue(forKey: downloadTask) else {
            return
        }
        if let error {
            continuation.resume(throwing: error)
        } else {
            // Rare: failure to report success via `didFinishDownloadingTo`; avoid force-unwrap crash from upstream.
    ...
    /// Cancels queued work and in-flight `URLSessionDownloadTask`s (user-initiated stop).
    func cancelAll() {
        let pending = queue
        queue.removeAll()
        for item in pending {
